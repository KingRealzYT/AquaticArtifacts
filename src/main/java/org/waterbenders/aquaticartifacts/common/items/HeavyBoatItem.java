...
public class HeavyBoatItem extends Item
{
    private static final Predicate<Entity> ENTITY_PREDICATE = EntityPredicates.NO_SPECTATORS.and(Entity::isPickable);

    public HeavyBoatItem() {
        super(new Properties().tab(AquaticArtifacts.TAB).stacksTo(1));
    }
    ...
    public ActionResult<ItemStack> use(World p_77659_1_, PlayerEntity p_77659_2_, Hand p_77659_3_) {
        ...
        if (raytraceresult.getType() == RayTraceResult.Type.MISS) {
            ...
            if (raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
                HeavyBoat boatentity = new HeavyBoat(p_77659_1_, raytraceresult.getLocation().x, raytraceresult.getLocation().y, raytraceresult.getLocation().z);
